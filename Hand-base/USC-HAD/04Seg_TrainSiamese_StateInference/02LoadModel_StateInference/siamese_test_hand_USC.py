...
parser.add_argument('--csiFile',   dest='csiFile', default='usc_2t5.mat', help='CSI data File Name')
...
def LoopTime():
    ...
    actNum = 1
    load_test_bath = 8
    # discretizeData = h5py.File(args.discretizeCsi + 'usc_2t5' + '.mat', 'r')
...
class generate_dataPairs:
    ...
    def generate_testPairs(self,load_test_length): #times代表划分成部分后的计数值，1代表第一部分，2代表第二部分,在main两次调用
        userNum=1
        actNum=1
        load_test_length=8
    ...
    #     discretizeData = h5py.File(args.discretizeCsi + 'usc_2t5' + '.mat', 'r')
        ...
        # discretizeLabel = h5py.File(args.test40Lab+'sample_40.mat', 'r')
        discretizeLabel = scio.loadmat(args.test40Lab + 'usc_t1.mat')
        discretizeData = discretizeData['data_']
        ...
        #DscData_l = np.zeros((lengthh*40,120,30,3))
                DscData_l = np.zeros((lengthh*28,3, 30, 120))
        #DscData_r = np.zeros((lengthh*40,120,30,3))
                DscData_r = np.zeros((lengthh*28,120, 30, 3))
                n=0
...
def normalizeData(testCsi_l, testCsi_r):
    ...
    for i in range(len(testCsi_l)):
        for j in range(3):
            nm_testCsi_l[i, :, :, j] = min_max_scaler.fit_transform(testCsi_l[i, :, :, j])  ###这一步以后，所有的数据都在0-1的范围内
...
class siamese_network():
    ...
    # 计算参数acc
    def test_pairs(self, testCsi_l, testCsi_r, n_way=28):
         correct_pred = 0
         X_l = testCsi_l
         X_r = testCsi_r
         j = 0

         # siamese_predict_ema = open(args.outputDir + '/'+args.testCsi.replace(".mat", "") + "_predict_ema", "a") ###写预测文件
         siamese_predict_ema = open(args.outputDir + '/' + args.testCsi.replace(".mat", "") + "_" + args.csiFile.replace(".mat", "") + "_predict_ema","a")  ###写预测文件


         for i in range(0,len(X_l),n_way):         ###n_way表示步长
             X_left, X_right = X_l[i: i + n_way], X_r[i: i + n_way]
             X_left, X_right = np.array(X_left), np.array(X_right)
             predictSeg, prob = self.test_one_shot(X_left, X_right)  ##2021.5.01 predictSeg为预测的动作类别（1,2,3,4）
 ...
 ###只输出类别 5.10
    ...
    def test_one_shot(self, X_left, X_right):
        ...
        predictSeg = np.argmax(np.bincount(predict3Seg))+1          ###求最大的三个元素中出现次数最多的元素的下标,+1是因为从0开始
        ###修正因标签顺序引起的乱码问题2021.7.26
        if predictSeg==1:
            predictSeg=2
        elif predictSeg==2:
            predictSeg=3
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(loopTime)):
        ...
        (predictSeg,prob) = model.test_validation_acc(test_l,test_r, n_way=28)
