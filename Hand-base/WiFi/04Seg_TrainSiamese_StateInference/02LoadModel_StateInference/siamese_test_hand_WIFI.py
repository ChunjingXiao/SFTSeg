...
parser.add_argument('--csiFile',   dest='csiFile', default='user1_rp_6.mat', help='CSI data File Name')
...
def LoopTime():
    ...
    actNum = 6
    load_test_bath = 8
    #discretizeData = h5py.File(args.discretizeCsi + 'user' + str(userNum) + '_iw_' + str(actNum) + '.mat', 'r')
...
class generate_dataPairs:
    ...
    def generate_testPairs(self,load_test_length): #times代表划分成部分后的计数值，1代表第一部分，2代表第二部分,在main两次调用
        ...
        discretizeData = discretizeData['data_']
        discretizeLabel = discretizeLabel['sample_40']
        ...
        while(True):
            for i in range(int(discretizeData.shape[0]/load_test_length)):
                ...
                DscData_l = np.zeros((lengthh*40,3, 30, 120))

                DscData_r = np.zeros((lengthh*40,3, 30, 120))
                n=0
...
class siamese_network():
    ...
    # 计算参数acc
    def test_pairs(self, testCsi_l, testCsi_r, n_way=40):
         correct_pred = 0
         X_l = testCsi_l
         X_r = testCsi_r
         j = 0

         siamese_predict_ema = open(args.outputDir + '/'+args.testCsi.replace(".mat", "") + args.csiFile.replace(".mat", "") + "_predict_ema", "a") ###写预测文件
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(loopTime)):
        ...
        (predictSeg,prob) = model.test_validation_acc(test_l,test_r, n_way=40)
