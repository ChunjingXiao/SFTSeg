...
def actionStartEndPoints(predictDataDir):
    ...
    qDetectChange = queue.Queue(maxsize=10) #create a queue with length=10 for detecting whether state labels are changed
    for i in range(7):
    ...
    startEndMark = np.zeros([7,2]) # startEndMark[0,0] is the start,  and startEndMark[0,1] is the end of the first activity
    lastMode = 0    
    currentMode = 0
    whichSample = 0  # which activity of 11 activities
    oneSampleStartMark = False
    for i in range(7,len(stateLabels)):
        ...
        if(lastMode != currentMode and currentMode == 1): #----detect end points
            if(oneSampleStartMark):
                ...
                if((modePrev1 == 4 or modePrev2 == 4) and (modeNext1 == 1 and modeNext2==1)  and dominatorRatePrev > dominatorRateThrehold):
                        ...
                        if(whichSample >= 10):               #-----for--------user5_ph_1
